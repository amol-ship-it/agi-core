...
For each task, shows:
- Training examples (input → expected output)
- Test examples: our prediction vs expected output (with diff highlighting)
...
.step-label { color: #7FDBFF; font-size: 0.85em; min-width: 200px; }
...
.filter-bar a { color: #7FDBFF; margin-right: 15px; text-decoration: none; }
...
def render_grid(grid: list[list[int]], diff_grid: Optional[list[list[int]]] = None,
    ...
    for r in range(h):
        for c in range(w):
            ...
            is_diff = diff_grid is not None and int(arr[r, c]) != int(np.array(diff_grid)[r, c])
...
def collect_intermediate_steps(prog: Program, grid: list[list[int]],
    ...
    def _walk(node: Program, current_grid: list[list[int]]) -> list[list[int]]:
        # First process children
        child_grid = current_grid
        if node.children:
            child_grid = _walk(node.children[0], current_grid)

        # Apply this node's primitive
        prim = _PRIM_MAP.get(node.root)
        if prim is None or prim.arity == 0:
            return child_grid
...
def generate_html(results_path: str, output_path: str,
    ...
    parts.append(f'<h1>ARC-AGI Results Visualization</h1>')
    ...
    for key, tid, tdata, status, err in task_items:
        ...
        parts.append(f'<div class="task {status}">')
        parts.append(f'<div class="task-header">')
        parts.append(f'<span class="task-id">{html.escape(tid)}</span>')
        ...
        # Training examples
        parts.append('<div class="section-label">Training Examples</div>')
        parts.append('<div class="examples">')
        for i, (inp, exp) in enumerate(task.train_examples):
            parts.append('<div class="example">')
            parts.append(f'<div>{render_grid(inp)}<div class="example-label">Train {i+1} Input</div></div>')
            parts.append('<div class="arrow">→</div>')
            parts.append(f'<div>{render_grid(exp)}<div class="example-label">Expected</div></div>')
            parts.append('</div>')
        parts.append('</div>')
        ...
        if task.test_inputs:
            ...
            for i, test_inp in enumerate(task.test_inputs):
                ...
                # Get our prediction
                try:
                    prediction = env.execute(prog, test_inp)
                except Exception:
                    prediction = test_inp

                parts.append('<div class="examples">')
                parts.append('<div class="example">')
                parts.append(f'<div>{render_grid(test_inp)}<div class="example-label">Test {i+1} Input</div></div>')
                parts.append('<div class="arrow">→</div>')

                # Our prediction
                if test_exp is not None:
                    pred_arr = np.array(prediction, dtype=np.int32)
                    exp_arr = np.array(test_exp, dtype=np.int32)
                    ...
                    # Show diff highlighting if shapes match
                    diff_ref = test_exp if pred_arr.shape == exp_arr.shape else None
                    ...
                    parts.append(f'<div>{render_grid(test_exp)}'
                ...
                if prog.children:
                    ...
                    if len(steps) > 2:  # more than just input→output
                        parts.append('<div class="section-label">Intermediate Steps</div>')
